package com.xienan.nio.nioserver;
...
public class Server implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                if (num > 0) {
                	System.out.println("get selectors " + num);
                    Set selectedKeys = selector.selectedKeys();
                    Iterator it = selectedKeys.iterator();
                    while (it.hasNext()) {
                        SelectionKey key = (SelectionKey) it.next();
                        System.out.println("select " + key.readyOps());
                        it.remove();
                        // ����IO�¼�
                        if ( (key.readyOps() & SelectionKey.OP_ACCEPT) == SelectionKey.OP_ACCEPT) {
                           // Accept the new connection
                           ServerSocketChannel ssc = (ServerSocketChannel) key.channel();
                           if (ssc.equals(sschannel)) {	
                        	   System.out.println("server socket does not change from key.channel");
                           }
                           notifier.fireOnAccept();

                           SocketChannel sc = ssc.accept();
							if (sc.equals(ssc)) {
								System.out.println("server socket does not change after accept");
							} else {
								System.out.println("server socket changed after accept");
							}
                           ...
                           Reader.processRequest(key);  // �ύ�w����̶߳�ȡ�ͻ������
                           key.cancel();
                       }
    ...
    public static void main(String[] args) throws Exception{
    	Server server = new Server(51641);
